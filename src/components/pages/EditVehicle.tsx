    ...
    return(
        <div className="fixed inset-0 shadow-xl flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                <h1 className="text-xl font-bold mb-4">Edit Vehicle - {vehicleData.id}</h1>
                <form onSubmit={submit} className="space-y-3">
                    <div>
                        <label htmlFor="year" className="block text-sm font-medium">Year</label>
                        <input type="number" name="year" id="year" defaultValue={vehicleData.year} className="mt-1 block w-full border rounded-md p-2" onChange={e => setYear(Number(e.target.value))} required />
                    </div>
